...
func main() {
    ...
    // Observability first so subsequent connect-failure logs already carry
    // service=worker-standard label (Loki / Promtail picks it up).
    obs, shutdown, err := observability.Init(ctx, "worker-standard", metricsAddr, otlpEndpoint)
    if err != nil {
