...
## 12. Stage 4 — 可观测性（2026-05-13 ~ 2026-05-15）

**目标**：把"黑盒"系统变成能看到、能告警、能跨服务追踪的系统。

### 12.1 完成的 8 个 Phase

| Phase | 日期 | 交付物 |
|---|---|---|
| 1. 装可观测性基础设施 | 05-13 | Helm 依赖加 kube-prometheus-stack 58.2.1 / loki-stack 2.10.2 / tempo 1.10.1 |
| 2. 构建 `internal/observability` 包 | 05-13 | observability.go (Init)、metrics.go (12 个 collector)、gin_middleware.go |
| 3. 接入 API + 3 worker | 05-13 | api/worker 各自起 /metrics 端点；servicemonitors.yaml 让 Prom scrape |
| 4. Kafka trace 传播 | 05-13 | trace_carrier.go 用 W3C traceparent 在 Kafka header 里传 span context |
| 5. JSON 日志 + trace_id 注入 | 05-15 | log_with_trace.go 的 `WithTrace(ctx, log)`；5 个 handler 全部接入；Promtail JSON pipeline |
| 6. Grafana 3 块 dashboard | 05-15 | Operator (4 panel) / Engineer (11 panel) / Capacity (6 panel)，ConfigMap + sidecar 自动加载 |
| 7. Alertmanager 规则 | 05-15 | PrometheusRule 4 recording + 5 alert；多窗口 SLO burn-rate；mock-webhook 接收 |
| 8. DoD 收尾 | 05-15 | 验证 SLOFastBurn firing → mock-webhook 收 payload |

### 12.2 期间踩的关键坑（细节在 `Processing_Platform_project_docs/docs/technical_issues.md`）

- **#8**：Loki 查询写 `<UUID>` 占位符当字面值传 → 解析报错 (literal not terminated)
- **#9**：loki-stack chart 默认 Promtail push 到 `http://pp:3100`，但 service 是 `pp-loki:3100` → 整个 Loki 收不到一行日志，Grafana 报 "No labels found"。`config.lokiAddress` 简写键在 loki-stack 2.10.2 里被静默忽略；必须覆盖 `config.clients[]` 数组
- **#10**：PrometheusRule 模板 `{{ $labels.tier }}` 被 Helm 当变量解析失败。修：用 backtick literal `` {{`{{ $labels.tier }}`}} `` 转义。**Helm 连 YAML 注释也会解析**
- **#11**：**worker.go 把 retry 调度成功算作 job 成功** → `pp_jobs_processed_total{state="failed"}` 永远是 0 → SLO alert 永远 fire 不了。从 Stage 3 起就有的 bug，**直到 Stage 4 Phase 7 用 SLO alert 真去触发才暴露**。修：processOne 始终返回 handlerErr

### 12.3 当前可观测能力

```
3 块 Grafana dashboard：Operator (3 秒看是否着火) / Engineer (排查瓶颈) / Capacity (容量规划)
5 条 SLO + 运维告警：SLOFastBurn / SLOSlowBurn / DLQNotEmpty / ReaperStalled / DBPoolHigh
3 条数据通路：
  Metrics: app → /metrics → ServiceMonitor → Prometheus → Grafana
  Logs:    app stdout (JSON) → Promtail (json+labels) → Loki → Grafana
  Traces:  app → OTLP gRPC → Tempo → Grafana
```

trace_id 把 metric / log / trace 三条线串起来——从 Tempo 一个 span 能跳到 Loki 查同 trace_id 的日志。

---

## 13. Stage 5 — Autoscaling + Load Testing（2026-05-17 ~ 进行中）

**目标**：让系统"扛得住事"——HPA 自动扩缩、大流量 load test、过载时优雅降级。

### 13.1 完成进度

| Phase | 日期 | 交付物 | 状态 |
|---|---|---|---|
| 1. Kafka lag 数据源 | 05-17 | `cmd/kafka-lag-exporter` (~120 行 Go) + subchart；Prom 真有 `pp_kafka_consumer_lag` 数据 | ✅ |
| 2. prometheus-adapter + HPA | 05-17 | adapter 装好、External Metrics API 暴露 `kafka_consumer_lag`；3 个 HPA 按 tier 各自 watch 自己 topic 的 lag；scaleUp 0s 立刻扩 / scaleDown 300s 防 flap | ✅ |
| 3. resource requests/limits + PDB | 05-18 | 3 个 worker 重设 resources；3 个 PDB minAvailable=1；drain 测试验证 PDB 真挡 | ✅ |
| 4. devsim 设备模拟器 | 05-18 | `cmd/devsim` (~150 行 Go) + subchart；100/1000 设备 MQTT publish；自带 Prom metrics | ✅ |
| 5. k6 HTTP 流量发生器 | 05-19 | `test/load/submit-rest.js` (ramp 0→1000 RPS, 11min) + `read-rest.js` | ✅ |
| 6. `make load-test` 编排 | 05-19 | Makefile target 写好；修了 `kubectl scale --quiet` flag bug | 🟡 |
| 7. Backpressure | 05-19 | `internal/lagcache/`（cache + middleware）；API 周期拉 Prom lag；POST /jobs 上 lag > tier 阈值 → 503 + Retry-After | ✅ 代码完成 |
| 8. DoD 验证 | — | 五种 task type SLO 验证、reaper-under-load、Grafana 截图、写 load-test-report | ⏳ |

### 13.2 期间的关键发现（细节在 `technical_issues.md` #12 - #13）

**#12 ingestion 漏改 JobSubmitter 7 天后才暴露**：
- Stage 3 Phase 6 doc 说要把 ingestion 改用 JobSubmitter 走 CreateJob + Publish + UpdateState 三步，但实际代码里只改了 JobSubmitter 自身和 3 个 worker，**ingestion 这个 caller 漏了**
- Phase 4 devsim 大流量灌入时 worker 报 `get job: no rows in result set`
- 修：ingestion 加 store 依赖 + jobsubmitter.New(producer, st)；values.yaml 加 POSTGRES_DSN
- 教训：**doc 跟代码可能悄悄不一致**，跨 phase 的 contract 修复必须扫所有 caller

**#13 一次 debug session 暴露 5 个独立 root cause**（2026-05-18 同日）：
1. `rate([1m])` 返回 empty —— PromQL rate 函数在新 metric 上 cold-start
2. ingestion 没 INSERT jobs 表 —— Stage 3 漏改的延续
3. helm upgrade 看似成功但 deployment env 没更新 —— `charts/ingestion-0.1.0.tgz` 老 tgz 优先于目录
4. Kafka 整集群挂 —— **磁盘 100% 满**（docker build cache 23.5 GB）；Phase 3 PDB 测试 drain 完忘 `uncordon`
5. ingestion `jobs_device_id_fkey` violation —— devsim 用 `dev-00xxx` 但 DB 只有 `device-001`

**#13 update 2/3 测试 fixture 跟生产契约不一致**（2026-05-19 同一类问题第 3 次）：
- k6 用 `dev-${num}` 但 DB 是 `dev-${5位 padding}` → FK violation 500
- k6 用 `Math.random()` 含小数点但 idempotency_key 正则 `^[A-Za-z0-9_-]{8,128}$` 不允许 → 400
- 根本性结论：**生产侧契约应集中文档化**，所有 fixture 生产者从同一处读

### 13.3 Stage 5 高 RPS 测试暴露的新 bug

**API OOMKilled under 500+ RPS**（2026-05-19）：
- API 原 limits cpu=500m / memory=256Mi，3min 跑到 ~500 RPS 时 exitCode 137（SIGKILL）
- 修：bump 到 cpu=1 / memory=1Gi
- **绕过 helm conflict**：HPA 接管 worker .spec.replicas，helm upgrade 撞 server-side apply 冲突。API 自己没 HPA，用 `kubectl patch deploy` 改 resources 直接绕过

**`kubectl port-forward` 在 500+ RPS 下崩溃**：
- 单 SPDY 连接撑不住高 throughput，加上 API 重启 port-forward 跟着死
- 缓解：跑 k6 在 cluster 里（K8s Job）连 `http://api:8080`，bypass port-forward 单点

### 13.4 系统承载能力（实测 2026-05-19）

- **MQTT 入口**：devsim 1000 设备，~200 msg/s 持续 publish
- **HTTP 入口**：API 单 pod (1Gi mem) 能稳定撑 ~400-500 RPS
- **Worker**：HPA 工作正常，realtime tier lag > 30 触发扩到 maxReplicas=3
- **观测性全链路**：trace_id 从 API/ingestion → Kafka → worker handler 都能串起来

### 13.5 portfolio 讲述主线

```
Stage 2 → 单 tier 端到端最短路径
Stage 3 → 5 个 task type / 3 个 tier / retry + DLQ + reaper
Stage 4 → 全可观测：metric + log + trace 用 trace_id 串
Stage 5 → 抗压：lag-based HPA + load test + backpressure
```

**最值得讲的 3 个"我懂分布式系统"信号**：
1. **Lag-based HPA**（不是 CPU-based）——证明懂 IO-bound 服务的"负载"定义
2. **Multi-window SLO burn-rate alert**（不是单阈值）——证明懂 SRE Workbook
3. **Backpressure with `Retry-After`**（不是任由 OOM）——证明懂系统过载的优雅降级

### 13.7 完整 11min load test 实测（2026-05-19）

详细数据 + 截图清单见 `docs/load-test-report.md`。摘要：

**端到端数据**：
- k6 跑 11m 30s，ramping 0 → 1000 RPS，93,887 iterations 完成
- **78% 失败率**（其中 <2% 真错误，**76% 是 backpressure 主动 503**）
- 单 API pod 实测上限：~200-250 RPS（k6 想 push 1000，实际处理 ~136 RPS）
- p99 latency: realtime tier 3.06s（> 2s SLO，bound by backpressure）

**HPA 表现**：
| Tier | initial → max | 触发时机 |
|---|---|---|
| worker-realtime | 1 → 3 | T+1min (lag=426 > 30 threshold) |
| worker-standard | 1 → 3 | T+5min (lag=1567 > 200) |
| worker-bulk | 1 → 2 | T+1min (lag=1152 > 500) |

3 个 tier 全部按设计达到 maxReplicas，**HPA lag-based 决策完全 working**。

**Backpressure 表现**：
- 每个时间点 503 占比 65-71%，几乎全是 bulk + realtime tier（lag 超阈值）
- standard tier 在大部分时间内 lag < 1000 → 流量 202
- **同时间 500（真错误）占比仅 0-1.8%**，证明系统没真崩，只是主动限流

**为什么 SLO 不达标**：
- 单 API pod (1Gi mem) + 单核 CPU 在 kind 本地撑不住 1000 RPS
- Postgres pgxpool 默认 25 connections 不够（每条请求要 INSERT + Redis SETNX + Kafka publish）
- **不是软件设计问题，是硬件/规模问题**——上 cloud + 多副本就解了

**这次 load test 暴露的工程教训**（已记 technical_issues.md）：
1. `kubectl port-forward` 在 500+ RPS 下死掉 → 改用 K8s Job 跑 k6 in-cluster
2. k6 fixture 跟 API validator 契约不一致（device_id padding、idempotency_key 正则）→ #13 update
3. API 资源 limit 在空载估值下不够 1000 RPS → bump 到 1Gi
4. helm upgrade 跟 HPA 抢 `.spec.replicas` → 用 `kubectl patch` 绕过

### 13.8 Stage 5 portfolio 5 个硬证据

1. HPA 不靠 CPU 也能扩 — lag-based 决策对 IO-bound 服务才对
2. Backpressure 救命 — 78% 失败 vs 2% 真错误，主动限流避免雪崩
3. Tier 隔离 — standard tier 不被 realtime/bulk 过载拖垮
4. 吞吐上限是可观察的 — 实测 ~250 RPS 是真硬数据，可以谈 scaling story
5. Load test 本身是工程 — 暴露了 4 个真实生产坑（port-forward、fixture 契约、资源 limit、helm 冲突）
